...
[[nodiscard]] stat::Metadata to_static( Metadata const& s, [[maybe_unused]] core::settings::StaticSettingsDataStorage& data_storage, [[maybe_unused]] void ** data_offset_pp )
{
    auto src_flow_path = s.src_flow_path();
    auto src_flow_path_data_p = static_cast< char * >( *data_offset_pp );
    auto src_flow_path_size = src_flow_path.size() + 1ull;
    std::memcpy( *data_offset_pp, src_flow_path.data(), src_flow_path_size );
    *data_offset_pp = static_cast< void * >( & ( static_cast< char * >( src_flow_path_data_p )[ src_flow_path_size ] ) );
    
    auto src_part_path = s.src_part_path();
    auto src_part_path_data_p = static_cast< char * >( *data_offset_pp );
    auto src_part_path_size = src_part_path.size() + 1ull;
    std::memcpy( *data_offset_pp, src_part_path.data(), src_part_path_size );
    *data_offset_pp = static_cast< void * >( & ( static_cast< char * >( src_part_path_data_p )[ src_part_path_size ] ) );
